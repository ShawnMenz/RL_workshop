...
# Path config
IMG_PATH = "./img/"
EPISODE_PATH = "./img/sarsa/"
OPTIMAL_GRID_PATH = "./img/sarsa_optimal_path.png"
...
for episode in range(episodes):
    ...
    while True:
        ...
        # next action
        if np.random.rand() < epsilon:
            next_action = np.random.choice(num_action)
        else:
            next_action = np.argmax(Q_table[next_state_in_table])

        Q_table[current_state_in_table, action] = Q_table[
            current_state_in_table, action
        ] + lr * (
            reward
            + discount * Q_table[next_state_in_table, next_action]
